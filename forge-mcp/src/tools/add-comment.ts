...
export function addCommentTool(
  ...
  agent: AgentIdentity,
): ForgeTool<typeof addCommentSchema.shape> {
  return {
    name: 'add_comment',
    description:
      "Add a comment to a ticket's activity log. Use this to log progress or flag a blocker while working on a ticket. The author is taken from your API key.",
    ...
    handler: async (input) => {
      ...
      const comment = await repo.addComment(ticket.id, input.body, agent);
